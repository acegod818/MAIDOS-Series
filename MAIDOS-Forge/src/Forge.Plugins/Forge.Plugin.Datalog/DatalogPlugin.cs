using Forge.Core;
// MAIDOS-Forge Datalog Language Plugin
...
namespace Forge.Plugin.Datalog;

/// <summary>Datalog 語言插件 - Datalog with Souffle</summary>
public sealed class DatalogPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".dl", ".datalog" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("souffle"))
            return (false, "souffle not found. Install from https://souffle-lang.github.io/");
        var version = await ProcessRunner.GetVersionAsync("souffle", "--version");
        return (true, $"souffle {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Datalog] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("souffle", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "datalog", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "datalog", ct)).ToArray()
        });
