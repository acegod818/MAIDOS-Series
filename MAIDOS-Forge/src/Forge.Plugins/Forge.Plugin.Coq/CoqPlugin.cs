using Forge.Core;
// MAIDOS-Forge Coq Language Plugin
...
namespace Forge.Plugin.Coq;

/// <summary>Coq 語言插件 - Coq proof assistant</summary>
public sealed class CoqPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".v" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("coqc"))
            return (false, "coqc not found. Install from https://coq.inria.fr/");
        var version = await ProcessRunner.GetVersionAsync("coqc", "--version");
        return (true, $"coqc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".vo");
            logs.Add($"[Coq] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("coqc", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "coq", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "coq", ct)).ToArray()
        });
