...
public class testPluginTests
{
    ...
    {
        var plugin = new Forge.Plugin.test.testPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("test", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".test"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "test", Abi = "c" },
