...
public class forthPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.forth.forthPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("forth", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".forth"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "forth", Abi = "c" },
