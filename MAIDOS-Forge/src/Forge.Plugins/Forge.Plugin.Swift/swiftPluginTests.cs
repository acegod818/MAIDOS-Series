...
public class swiftPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.swift.swiftPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("swift", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".swift"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "swift", Abi = "c" },
