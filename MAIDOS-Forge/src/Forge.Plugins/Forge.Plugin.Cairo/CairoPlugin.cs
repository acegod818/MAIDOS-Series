using Forge.Core;
// MAIDOS-Forge Cairo Language Plugin
...
namespace Forge.Plugin.Cairo;

/// <summary>Cairo language plugin</summary>
public sealed class CairoPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".cairo" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("cairo-compile"))
            return (false, "cairo-compile not found. Install from https://www.cairo-lang.org/docs/quickstart.html");
        var version = await ProcessRunner.GetVersionAsync("cairo-compile", "--version");
        return (true, $"cairo-compile {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Cairo] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("cairo-compile", $"\"{f}\" --output \"{outDir}/{bn}.json\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "cairo", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "cairo", ct)).ToArray()
        });
