using Forge.Core;
// MAIDOS-Forge Lua Language Plugin
...
namespace Forge.Plugin.Lua;

/// <summary>Lua language plugin</summary>
public sealed class LuaPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".lua" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("luac"))
            return (false, "luac not found. Install from https://www.lua.org/download.html");
        var version = await ProcessRunner.GetVersionAsync("luac", "--version");
        return (true, $"luac {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".luac");
            logs.Add($"[Lua] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("luac", $"-o \"{outDir}/{bn}.luac\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "lua", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "lua", ct)).ToArray()
        });
