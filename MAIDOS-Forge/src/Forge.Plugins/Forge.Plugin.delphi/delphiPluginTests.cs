...
public class delphiPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.delphi.delphiPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("delphi", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".delphi"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "delphi", Abi = "c" },
