...
public class lispPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.lisp.lispPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("lisp", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".lisp"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "lisp", Abi = "c" },
