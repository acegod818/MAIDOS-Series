using Forge.Core;
// MAIDOS-Forge Lisp Language Plugin
...
namespace Forge.Plugin.lisp;

/// <summary>Lisp language plugin</summary>
public sealed class LispPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".lisp", ".cl", ".lsp" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("sbcl"))
            return (false, "sbcl not found. Install from https://www.sbcl.org/platform-table.html");
        var version = await ProcessRunner.GetVersionAsync("sbcl", "--version");
        return (true, $"sbcl {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Lisp] Validating: {fn}");

            var r = await ProcessRunner.RunAsync("sbcl", $"--script \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "lisp", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "lisp", ct)).ToArray()
        });
