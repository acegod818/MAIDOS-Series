...
public class scalaPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.scala.scalaPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("scala", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".scala"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "scala", Abi = "c" },
