using Forge.Core;
// MAIDOS-Forge Objective-C Language Plugin
...
namespace Forge.Plugin.ObjectiveC;

/// <summary>Objective-C language plugin</summary>
public sealed class ObjectiveCPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".m", ".h" };
    ...
    public async Task<(bool Available, string Message)> ValidateToolchainAsync(CancellationToken ct = default)
    {
        if (!await ProcessRunner.CommandExistsAsync("clang"))
            return (false, "clang not found. Install from https://releases.llvm.org/");
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        logs.Add($"[Objective-C] Using: {msg}");
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Objective-C] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("clang", $"-ObjC -o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "objective-c", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "objectivec", ct)).ToArray()
        });
