...
public class pascalPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.pascal.pascalPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("pascal", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".pascal"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "pascal", Abi = "c" },
