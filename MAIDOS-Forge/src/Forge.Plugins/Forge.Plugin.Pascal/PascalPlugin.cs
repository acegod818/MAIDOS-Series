using Forge.Core;
// MAIDOS-Forge Pascal Language Plugin
...
namespace Forge.Plugin.Pascal;

/// <summary>Pascal language plugin</summary>
public sealed class PascalPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".pas", ".pp" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("fpc"))
            return (false, "fpc not found. Install from https://www.freepascal.org/download.html");
        var version = await ProcessRunner.GetVersionAsync("fpc", "-iV");
        return (true, $"fpc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Pascal] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("fpc", $"-o\"{outDir}/{bn}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "pascal", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "pascal", ct)).ToArray()
        });
