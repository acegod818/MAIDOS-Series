using Forge.Core;
// MAIDOS-Forge Prolog Language Plugin
...
namespace Forge.Plugin.Prolog;

/// <summary>Prolog language plugin</summary>
public sealed class PrologPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".pl", ".pro" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("swipl"))
            return (false, "swipl not found. Install from https://www.swi-prolog.org/download/stable");
        var version = await ProcessRunner.GetVersionAsync("swipl", "--version");
        return (true, $"swipl {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Prolog] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("swipl", $"-o \"{outFile}\" -c \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "prolog", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "prolog", ct)).ToArray()
        });
