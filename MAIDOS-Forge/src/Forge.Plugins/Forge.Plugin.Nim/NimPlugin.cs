using Forge.Core;
// MAIDOS-Forge Nim Language Plugin
...
namespace Forge.Plugin.Nim;

/// <summary>Nim language plugin</summary>
public sealed class NimPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".nim", ".nims" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("nim"))
            return (false, "nim not found. Install from https://nim-lang.org/install.html");
        var version = await ProcessRunner.GetVersionAsync("nim", "--version");
        return (true, $"nim {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Nim] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("nim", $"compile --out:\"{outDir}/{bn}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "nim", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "nim", ct)).ToArray()
        });
