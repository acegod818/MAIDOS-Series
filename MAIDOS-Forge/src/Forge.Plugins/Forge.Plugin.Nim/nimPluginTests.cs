...
public class nimPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.nim.nimPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("nim", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".nim"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "nim", Abi = "c" },
