using Forge.Core;
// MAIDOS-Forge Carbon Language Plugin
...
namespace Forge.Plugin.Carbon;

/// <summary>Carbon 語言插件 - Carbon successor to C++</summary>
public sealed class CarbonPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".carbon" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("carbon"))
            return (false, "carbon not found. Install from https://github.com/carbon-language/carbon-lang");
        var version = await ProcessRunner.GetVersionAsync("carbon", "--version");
        return (true, $"carbon {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Carbon] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("carbon", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "carbon", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "carbon", ct)).ToArray()
        });
