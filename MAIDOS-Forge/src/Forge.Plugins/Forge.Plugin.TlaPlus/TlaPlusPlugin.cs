using Forge.Core;
// MAIDOS-Forge TLAPlus Language Plugin
...
namespace Forge.Plugin.TlaPlus;

/// <summary>TLAPlus 語言插件 - TLA+ specification language</summary>
public sealed class TlaPlusPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".tla" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("tla2tools"))
            return (false, "tla2tools not found. Install from https://lamport.azurewebsites.net/tla/tla.html");
        var version = await ProcessRunner.GetVersionAsync("tla2tools", "--version");
        return (true, $"tla2tools {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".verified");
            logs.Add($"[TLAPlus] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("tla2tools", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "tlaplus", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "tlaplus", ct)).ToArray()
        });
