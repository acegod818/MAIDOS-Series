using Forge.Core;
// MAIDOS-Forge Lean Language Plugin
...
namespace Forge.Plugin.Lean;

/// <summary>Lean 語言插件 - Lean theorem prover</summary>
public sealed class LeanPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".lean" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("lean"))
            return (false, "lean not found. Install from https://leanprover.github.io/");
        var version = await ProcessRunner.GetVersionAsync("lean", "--version");
        return (true, $"lean {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".olean");
            logs.Add($"[Lean] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("lean", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "lean", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "lean", ct)).ToArray()
        });
