using Forge.Core;
// MAIDOS-Forge Pkl Language Plugin
...
namespace Forge.Plugin.Pkl;

/// <summary>Pkl 語言插件 - Apple Pkl configuration</summary>
public sealed class PklPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".pkl" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("pkl"))
            return (false, "pkl not found. Install from https://pkl-lang.org/");
        var version = await ProcessRunner.GetVersionAsync("pkl", "--version");
        return (true, $"pkl {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Pkl] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("pkl", $"eval \"{f}\" -o \"{outDir}/{bn}.json\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "pkl", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "pkl", ct)).ToArray()
        });
