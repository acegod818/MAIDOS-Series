using Forge.Core;
// MAIDOS-Forge AsyncAPI Language Plugin
...
namespace Forge.Plugin.AsyncApi;

/// <summary>AsyncAPI language plugin</summary>
public sealed class AsyncApiPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".yaml", ".yml", ".json" };

    public PluginCapabilities GetCapabilities() => new()
    {
        LanguageName = "asyncapi",
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("asyncapi"))
            return (false, "asyncapi not found. Install from https://www.asyncapi.com/tools/cli");
        var version = await ProcessRunner.GetVersionAsync("asyncapi", "--version");
        return (true, $"asyncapi {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[AsyncAPI] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("asyncapi", $"generate fromTemplate \"{f}\" @asyncapi/html-template -o \"{outDir}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "asyncapi", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "asyncapi", ct)).ToArray()
        });
