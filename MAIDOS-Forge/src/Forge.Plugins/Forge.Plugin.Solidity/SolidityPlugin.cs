using Forge.Core;
// MAIDOS-Forge Solidity Language Plugin
...
namespace Forge.Plugin.Solidity;

/// <summary>Solidity language plugin</summary>
public sealed class SolidityPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".sol" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("solc"))
            return (false, "solc not found. Install from https://docs.soliditylang.org/en/latest/installing-solidity.html");
        var version = await ProcessRunner.GetVersionAsync("solc", "--version");
        return (true, $"solc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Solidity] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("solc", $"--bin --abi -o \"{outDir}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "solidity", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "solidity", ct)).ToArray()
        });
