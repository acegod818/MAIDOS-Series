using Forge.Core;
// MAIDOS-Forge Sed Language Plugin
...
namespace Forge.Plugin.Sed;

/// <summary>Sed 語言插件 - Stream editor</summary>
public sealed class SedPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".sed" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("sed"))
            return (false, "sed not found. Install from https://www.gnu.org/software/sed/");
        var version = await ProcessRunner.GetVersionAsync("sed", "--version");
        return (true, $"sed {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Sed] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("sed", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "sed", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "sed", ct)).ToArray()
        });
