...
namespace Forge.Plugin.Jsonnet;

/// <summary>Jsonnet 語言插件 - Jsonnet data templating</summary>
public sealed class JsonnetPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".jsonnet", ".libsonnet" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("jsonnet"))
            return (false, "jsonnet not found. Install from https://jsonnet.org/");
        var version = await ProcessRunner.GetVersionAsync("jsonnet", "--version");
        return (true, $"jsonnet {version}");
    }
