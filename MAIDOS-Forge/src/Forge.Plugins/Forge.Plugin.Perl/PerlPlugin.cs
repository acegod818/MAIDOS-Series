using Forge.Core;
// MAIDOS-Forge Perl Language Plugin
...
namespace Forge.Plugin.Perl;

/// <summary>Perl language plugin</summary>
public sealed class PerlPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".pl", ".pm" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("perl"))
            return (false, "perl not found. Install from https://www.perl.org/get.html");
        var version = await ProcessRunner.GetVersionAsync("perl", "--version");
        return (true, $"perl {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Perl] Syntax check: {fn}");

            var r = await ProcessRunner.RunAsync("perl", $"-c \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "perl", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "perl", ct)).ToArray()
        });
