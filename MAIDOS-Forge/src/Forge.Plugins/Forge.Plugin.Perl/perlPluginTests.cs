...
public class perlPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.perl.perlPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("perl", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".perl"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "perl", Abi = "c" },
