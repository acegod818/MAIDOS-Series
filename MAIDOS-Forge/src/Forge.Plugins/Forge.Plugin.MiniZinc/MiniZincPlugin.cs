using Forge.Core;
// MAIDOS-Forge MiniZinc Language Plugin
// Code-QC v2.2B Compliant | M11 Specialist Plugin - Logic Languages

using System.Text;
using Forge.Core.Config;
using Forge.Core.Platform;

namespace Forge.Plugin.MiniZinc;

/// <summary>MiniZinc 語言插件 - MiniZinc constraint modeling</summary>
public sealed class MiniZincPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".mzn", ".dzn" };

    public PluginCapabilities GetCapabilities() => new()
    {
        LanguageName = "minizinc",
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("minizinc"))
            return (false, "minizinc not found. Install from https://www.minizinc.org/");
        var version = await ProcessRunner.GetVersionAsync("minizinc", "--version");
        return (true, $"minizinc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[MiniZinc] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("minizinc", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "minizinc", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "minizinc", ct)).ToArray()
        });
