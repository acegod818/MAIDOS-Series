using Forge.Core;
// MAIDOS-Forge Unison Language Plugin
...
namespace Forge.Plugin.Unison;

/// <summary>Unison 語言插件 - Unison content-addressed language</summary>
public sealed class UnisonPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".u" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("ucm"))
            return (false, "ucm not found. Install from https://www.unison-lang.org/");
        var version = await ProcessRunner.GetVersionAsync("ucm", "--version");
        return (true, $"ucm {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Unison] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("ucm", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "unison", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "unison", ct)).ToArray()
        });
