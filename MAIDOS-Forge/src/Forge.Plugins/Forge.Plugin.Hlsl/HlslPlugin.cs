using Forge.Core;
// MAIDOS-Forge HLSL Language Plugin
...
namespace Forge.Plugin.Hlsl;

/// <summary>HLSL 語言插件 - DirectX HLSL</summary>
public sealed class HlslPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".hlsl", ".fx" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("dxc"))
            return (false, "dxc not found. Install from https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/");
        var version = await ProcessRunner.GetVersionAsync("dxc", "--version");
        return (true, $"dxc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[HLSL] Processing: {fn}");

            var spvOut = Path.Combine(outDir, bn + ".spv");
            var r = await ProcessRunner.RunAsync("dxc", $"-V \"{f}\" -o \"{spvOut}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "hlsl", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "hlsl", ct)).ToArray()
        });
