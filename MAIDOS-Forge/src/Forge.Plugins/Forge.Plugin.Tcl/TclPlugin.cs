using Forge.Core;
// MAIDOS-Forge Tcl Language Plugin
...
namespace Forge.Plugin.Tcl;

/// <summary>Tcl 語言插件 - Tool Command Language</summary>
public sealed class TclPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".tcl" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("tclsh"))
            return (false, "tclsh not found. Install from https://www.tcl.tk/");
        var version = await ProcessRunner.GetVersionAsync("tclsh", "--version");
        return (true, $"tclsh {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Tcl] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("tclsh", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "tcl", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "tcl", ct)).ToArray()
        });
