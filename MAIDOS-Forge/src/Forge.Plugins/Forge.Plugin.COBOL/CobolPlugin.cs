using Forge.Core;
// MAIDOS-Forge COBOL Language Plugin
...
namespace Forge.Plugin.COBOL;

/// <summary>COBOL language plugin</summary>
public sealed class CobolPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".cob", ".cbl" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("cobc"))
            return (false, "cobc not found. Install from https://gnucobol.sourceforge.io/");
        var version = await ProcessRunner.GetVersionAsync("cobc", "--version");
        return (true, $"cobc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[COBOL] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("cobc", $"-x -o \"{outDir}/{bn}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "cobol", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "cobol", ct)).ToArray()
        });
