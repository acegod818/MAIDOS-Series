...
public class goPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.go.goPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("go", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".go"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "go", Abi = "c" },
