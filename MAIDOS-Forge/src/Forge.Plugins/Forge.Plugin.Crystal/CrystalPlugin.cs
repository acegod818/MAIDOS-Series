using Forge.Core;
// MAIDOS-Forge Crystal Language Plugin
...
namespace Forge.Plugin.Crystal;

/// <summary>Crystal language plugin</summary>
public sealed class CrystalPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".cr" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("crystal"))
            return (false, "crystal not found. Install from https://crystal-lang.org/install/");
        var version = await ProcessRunner.GetVersionAsync("crystal", "--version");
        return (true, $"crystal {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Crystal] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("crystal", $"build \"{f}\" -o \"{outDir}/{bn}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "crystal", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "crystal", ct)).ToArray()
        });
