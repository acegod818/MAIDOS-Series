...
public class haskellPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.haskell.haskellPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("haskell", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".haskell"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "haskell", Abi = "c" },
