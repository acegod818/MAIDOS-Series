using Forge.Core;
// MAIDOS-Forge WGSL Language Plugin
...
namespace Forge.Plugin.Wgsl;

/// <summary>WGSL 語言插件 - WebGPU Shading Language</summary>
public sealed class WgslPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".wgsl" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("naga"))
            return (false, "naga not found. Install from https://www.w3.org/TR/WGSL/");
        var version = await ProcessRunner.GetVersionAsync("naga", "--version");
        return (true, $"naga {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[WGSL] Processing: {fn}");

            var spvOut = Path.Combine(outDir, bn + ".spv");
            var r = await ProcessRunner.RunAsync("naga", $"-V \"{f}\" -o \"{spvOut}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "wgsl", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "wgsl", ct)).ToArray()
        });
