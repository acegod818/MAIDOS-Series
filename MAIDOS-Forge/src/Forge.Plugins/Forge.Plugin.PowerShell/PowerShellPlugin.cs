using Forge.Core;
// MAIDOS-Forge PowerShell Language Plugin
...
namespace Forge.Plugin.PowerShell;

/// <summary>PowerShell 語言插件 - PowerShell scripting</summary>
public sealed class PowerShellPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".ps1", ".psm1" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("pwsh"))
            return (false, "pwsh not found. Install from https://docs.microsoft.com/en-us/powershell/");
        var version = await ProcessRunner.GetVersionAsync("pwsh", "--version");
        return (true, $"pwsh {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[PowerShell] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("pwsh", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "powershell", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "powershell", ct)).ToArray()
        });
