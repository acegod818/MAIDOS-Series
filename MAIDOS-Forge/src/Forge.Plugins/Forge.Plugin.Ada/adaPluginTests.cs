...
public class adaPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.ada.adaPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("ada", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".ada"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "ada", Abi = "c" },
