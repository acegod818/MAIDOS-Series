using Forge.Core;
// MAIDOS-Forge Vyper Language Plugin
...
namespace Forge.Plugin.Vyper;

/// <summary>Vyper language plugin</summary>
public sealed class VyperPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".vy" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("vyper"))
            return (false, "vyper not found. Install from https://docs.vyperlang.org/en/stable/installing-vyper.html");
        var version = await ProcessRunner.GetVersionAsync("vyper", "--version");
        return (true, $"vyper {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Vyper] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("vyper", $"-o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "vyper", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "vyper", ct)).ToArray()
        });
