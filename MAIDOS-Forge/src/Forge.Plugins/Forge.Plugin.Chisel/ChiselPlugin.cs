using Forge.Core;
// MAIDOS-Forge Chisel Language Plugin
...
namespace Forge.Plugin.Chisel;

/// <summary>Chisel 語言插件 - Chisel hardware construction language</summary>
public sealed class ChiselPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".scala" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("sbt"))
            return (false, "sbt not found. Install from https://www.chisel-lang.org/");
        var version = await ProcessRunner.GetVersionAsync("sbt", "--version");
        return (true, $"sbt {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".jar");
            logs.Add($"[Chisel] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("sbt", $"-o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "chisel", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "chisel", ct)).ToArray()
        });
