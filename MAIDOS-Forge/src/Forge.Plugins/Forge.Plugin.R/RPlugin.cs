using Forge.Core;
// MAIDOS-Forge R Language Plugin
...
namespace Forge.Plugin.R;

/// <summary>R language plugin</summary>
public sealed class RPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".r", ".R" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("Rscript"))
            return (false, "Rscript not found. Install from https://cran.r-project.org/");
        var version = await ProcessRunner.GetVersionAsync("Rscript", "--version");
        return (true, $"Rscript {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[R] Validating: {fn}");

            var r = await ProcessRunner.RunAsync("Rscript", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "r", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "r", ct)).ToArray()
        });
