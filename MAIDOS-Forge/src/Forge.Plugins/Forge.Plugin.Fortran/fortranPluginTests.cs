...
public class fortranPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.fortran.fortranPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("fortran", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".fortran"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "fortran", Abi = "c" },
