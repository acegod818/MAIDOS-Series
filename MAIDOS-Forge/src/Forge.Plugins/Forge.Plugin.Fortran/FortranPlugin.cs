using Forge.Core;
// MAIDOS-Forge Fortran Language Plugin
...
namespace Forge.Plugin.Fortran;

/// <summary>Fortran language plugin</summary>
public sealed class FortranPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".f90", ".f95", ".f03" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("gfortran"))
            return (false, "gfortran not found. Install from https://gcc.gnu.org/wiki/GFortran");
        var version = await ProcessRunner.GetVersionAsync("gfortran", "--version");
        return (true, $"gfortran {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Fortran] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("gfortran", $"-o \"{outDir}/{bn}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "fortran", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "fortran", ct)).ToArray()
        });
