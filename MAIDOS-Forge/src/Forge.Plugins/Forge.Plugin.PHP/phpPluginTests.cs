...
public class phpPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.php.phpPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("php", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".php"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "php", Abi = "c" },
