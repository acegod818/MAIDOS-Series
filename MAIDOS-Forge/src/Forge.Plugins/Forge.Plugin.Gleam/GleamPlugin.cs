using Forge.Core;
// MAIDOS-Forge Gleam Language Plugin
...
namespace Forge.Plugin.Gleam;

/// <summary>Gleam 語言插件 - Gleam for BEAM VM</summary>
public sealed class GleamPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".gleam" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("gleam"))
            return (false, "gleam not found. Install from https://gleam.run/");
        var version = await ProcessRunner.GetVersionAsync("gleam", "--version");
        return (true, $"gleam {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Gleam] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("gleam", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "gleam", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "gleam", ct)).ToArray()
        });
