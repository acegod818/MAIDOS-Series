using Forge.Core;
// MAIDOS-Forge Vale Language Plugin
...
namespace Forge.Plugin.Vale;

/// <summary>Vale 語言插件 - Vale memory-safe language</summary>
public sealed class ValePlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".vale" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("vale"))
            return (false, "vale not found. Install from https://vale.dev/");
        var version = await ProcessRunner.GetVersionAsync("vale", "--version");
        return (true, $"vale {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Vale] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("vale", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "vale", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "vale", ct)).ToArray()
        });
