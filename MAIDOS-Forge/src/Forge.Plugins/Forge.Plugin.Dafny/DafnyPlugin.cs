using Forge.Core;
// MAIDOS-Forge Dafny Language Plugin
...
namespace Forge.Plugin.Dafny;

/// <summary>Dafny 語言插件 - Dafny verification language</summary>
public sealed class DafnyPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".dfy" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("dafny"))
            return (false, "dafny not found. Install from https://dafny.org/");
        var version = await ProcessRunner.GetVersionAsync("dafny", "--version");
        return (true, $"dafny {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Dafny] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("dafny", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "dafny", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "dafny", ct)).ToArray()
        });
