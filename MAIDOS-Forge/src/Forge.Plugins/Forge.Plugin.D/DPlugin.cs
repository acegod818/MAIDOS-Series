using Forge.Core;
// MAIDOS-Forge D Language Plugin
...
namespace Forge.Plugin.D;

/// <summary>D language plugin</summary>
public sealed class DPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".d" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("dmd"))
            return (false, "dmd not found. Install from https://dlang.org/download.html");
        var version = await ProcessRunner.GetVersionAsync("dmd", "--version");
        return (true, $"dmd {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[D] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("dmd", $"-of=\"{outDir}/{bn}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "d", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "d", ct)).ToArray()
        });
