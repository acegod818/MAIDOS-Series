using Forge.Core;
// MAIDOS-Forge MoonBit Language Plugin
...
namespace Forge.Plugin.MoonBit;

/// <summary>MoonBit 語言插件 - MoonBit for WebAssembly</summary>
public sealed class MoonBitPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".mbt" };

    public PluginCapabilities GetCapabilities() => new()
    {
        LanguageName = "moonbit",
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("moon"))
            return (false, "moon not found. Install from https://www.moonbitlang.com/");
        var version = await ProcessRunner.GetVersionAsync("moon", "--version");
        return (true, $"moon {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[MoonBit] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("moon", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "moonbit", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "moonbit", ct)).ToArray()
        });
