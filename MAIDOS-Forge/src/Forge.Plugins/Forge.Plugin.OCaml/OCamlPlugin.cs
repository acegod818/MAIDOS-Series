using Forge.Core;
// MAIDOS-Forge OCaml Language Plugin
...
namespace Forge.Plugin.OCaml;

/// <summary>OCaml language plugin</summary>
public sealed class OCamlPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".ml", ".mli" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("ocamlopt"))
            return (false, "ocamlopt not found. Install from https://ocaml.org/install");
        var version = await ProcessRunner.GetVersionAsync("ocamlopt", "--version");
        return (true, $"ocamlopt {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[OCaml] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("ocamlopt", $"-o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "ocaml", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "ocaml", ct)).ToArray()
        });
