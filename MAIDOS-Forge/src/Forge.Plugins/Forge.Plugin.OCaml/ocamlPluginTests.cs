...
public class ocamlPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.ocaml.ocamlPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("ocaml", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".ocaml"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "ocaml", Abi = "c" },
