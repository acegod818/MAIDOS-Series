...
public class typescriptPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.typescript.typescriptPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("typescript", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".ts"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "typescript", Abi = "c" },
