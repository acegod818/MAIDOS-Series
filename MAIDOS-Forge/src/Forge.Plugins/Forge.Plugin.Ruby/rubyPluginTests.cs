...
public class rubyPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.ruby.rubyPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("ruby", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".ruby"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "ruby", Abi = "c" },
