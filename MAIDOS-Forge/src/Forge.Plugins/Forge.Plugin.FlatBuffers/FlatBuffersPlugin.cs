using Forge.Core;
// MAIDOS-Forge FlatBuffers Language Plugin
...
namespace Forge.Plugin.FlatBuffers;

/// <summary>FlatBuffers language plugin</summary>
public sealed class FlatBuffersPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".fbs" };

    public PluginCapabilities GetCapabilities() => new()
    {
        LanguageName = "flatbuffers",
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("flatc"))
            return (false, "flatc not found. Install from https://google.github.io/flatbuffers/flatbuffers_guide_building.html");
        var version = await ProcessRunner.GetVersionAsync("flatc", "--version");
        return (true, $"flatc {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        logs.Add($"[FlatBuffers] Using: {msg}");
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[FlatBuffers] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("flatc", $"--csharp -o \"{outDir}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "flatbuffers", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "flatbuffers", ct)).ToArray()
        });
