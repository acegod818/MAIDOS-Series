using Forge.Core;
// MAIDOS-Forge CUE Language Plugin
...
namespace Forge.Plugin.Cue;

/// <summary>CUE 語言插件 - CUE configuration language</summary>
public sealed class CuePlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".cue" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("cue"))
            return (false, "cue not found. Install from https://cuelang.org/");
        var version = await ProcessRunner.GetVersionAsync("cue", "--version");
        return (true, $"cue {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[CUE] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("cue", $"export \"{f}\" --outfile \"{outDir}/{bn}.json\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "cue", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "cue", ct)).ToArray()
        });
