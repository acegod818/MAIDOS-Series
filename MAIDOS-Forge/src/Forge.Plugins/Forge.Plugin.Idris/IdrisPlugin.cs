using Forge.Core;
// MAIDOS-Forge Idris Language Plugin
...
namespace Forge.Plugin.Idris;

/// <summary>Idris 語言插件 - Idris dependently typed language</summary>
public sealed class IdrisPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".idr" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("idris2"))
            return (false, "idris2 not found. Install from https://www.idris-lang.org/");
        var version = await ProcessRunner.GetVersionAsync("idris2", "--version");
        return (true, $"idris2 {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn + ".ibc");
            logs.Add($"[Idris] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("idris2", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "idris", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "idris", ct)).ToArray()
        });
