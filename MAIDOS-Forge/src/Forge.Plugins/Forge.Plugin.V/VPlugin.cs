using Forge.Core;
// MAIDOS-Forge V Language Plugin
...
namespace Forge.Plugin.V;

/// <summary>V language plugin</summary>
public sealed class VPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".v", ".vv" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("v"))
            return (false, "v not found. Install from https://vlang.io/");
        var version = await ProcessRunner.GetVersionAsync("v", "--version");
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[V] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("v", $"-o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "v", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "v", ct)).ToArray()
        });
