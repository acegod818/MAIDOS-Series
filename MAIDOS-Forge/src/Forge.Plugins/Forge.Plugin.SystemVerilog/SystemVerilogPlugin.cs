using Forge.Core;
// MAIDOS-Forge SystemVerilog Language Plugin
...
namespace Forge.Plugin.SystemVerilog;

/// <summary>SystemVerilog 語言插件 - SystemVerilog for hardware verification</summary>
public sealed class SystemVerilogPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".sv", ".svh" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("verilator"))
            return (false, "verilator not found. Install from https://verilator.org/");
        var version = await ProcessRunner.GetVersionAsync("verilator", "--version");
        return (true, $"verilator {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        logs.Add($"[SystemVerilog] Using: {msg}");
        ...
        foreach (var f in files)
        {
            ...
            var outFile = Path.Combine(outDir, bn);
            logs.Add($"[SystemVerilog] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("verilator", $"-o \"{outDir}/{bn}\" -o \"{outFile}\" \"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "systemverilog", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "systemverilog", ct)).ToArray()
        });
