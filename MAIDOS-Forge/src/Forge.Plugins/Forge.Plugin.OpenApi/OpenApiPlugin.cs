using Forge.Core;
// MAIDOS-Forge OpenAPI Language Plugin
...
namespace Forge.Plugin.OpenApi;

/// <summary>OpenAPI language plugin</summary>
public sealed class OpenApiPlugin : ILanguagePlugin
{
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("openapi-generator"))
            return (false, "openapi-generator not found. Install from https://openapi-generator.tech/docs/installation/");
        var version = await ProcessRunner.GetVersionAsync("openapi-generator", "--version");
        return (true, $"openapi-generator {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[OpenAPI] Processing: {fn}");
            var r = await ProcessRunner.RunAsync("openapi-generator", $"generate -i \"{f}\" -o \"{outDir}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "openapi", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "openapi", ct)).ToArray()
        });
