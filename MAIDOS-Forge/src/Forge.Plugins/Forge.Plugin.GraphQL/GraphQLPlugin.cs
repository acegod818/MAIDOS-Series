using Forge.Core;
// MAIDOS-Forge GraphQL Language Plugin
// Code-QC v2.2B Compliant | M11 Specialist Plugin - Query Languages

using System.Text;
using Forge.Core.Config;
using Forge.Core.Platform;

namespace Forge.Plugin.GraphQL;

/// <summary>GraphQL 語言插件 - GraphQL query language</summary>
public sealed class GraphQLPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".graphql", ".gql" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("graphql-codegen"))
            return (false, "graphql-codegen not found. Install from https://graphql.org/");
        var version = await ProcessRunner.GetVersionAsync("graphql-codegen", "--version");
        return (true, $"graphql-codegen {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[GraphQL] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("graphql-codegen", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "graphql", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "graphql", ct)).ToArray()
        });
