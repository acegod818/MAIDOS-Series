...
public class juliaPluginTests
{
    public void Test_GetCapabilities()
    {
        var plugin = new Forge.Plugin.julia.juliaPlugin();
        var caps = plugin.GetCapabilities();

        Assert.Equal("julia", caps.LanguageName);
        Assert.True(caps.SupportedExtensions.Contains(".julia"));
    }
    ...
    public void Test_GenerateGlue_CSharp()
    {
        ...
        var iface = new Forge.Core.Plugin.InterfaceDescription
        {
            ...
            Language = new Forge.Core.Plugin.InterfaceLanguage { Name = "julia", Abi = "c" },
