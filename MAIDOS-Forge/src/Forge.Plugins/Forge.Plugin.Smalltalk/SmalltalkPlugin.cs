using Forge.Core;
// MAIDOS-Forge Smalltalk Language Plugin
...
namespace Forge.Plugin.Smalltalk;

/// <summary>Smalltalk language plugin</summary>
public sealed class SmalltalkPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".st" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("gst"))
            return (false, "gst not found. Install from https://www.gnu.org/software/smalltalk/");
        var version = await ProcessRunner.GetVersionAsync("gst", "--version");
        return (true, $"gst {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Smalltalk] Validating: {fn}");

            var r = await ProcessRunner.RunAsync("gst", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "smalltalk", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "smalltalk", ct)).ToArray()
        });
