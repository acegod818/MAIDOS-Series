using Forge.Core;
// MAIDOS-Forge LaTeX Language Plugin
...
namespace Forge.Plugin.Latex;

/// <summary>LaTeX 語言插件 - LaTeX typesetting</summary>
public sealed class LatexPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".tex", ".sty" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("pdflatex"))
            return (false, "pdflatex not found. Install from https://www.latex-project.org/");
        var version = await ProcessRunner.GetVersionAsync("pdflatex", "--version");
        return (true, $"pdflatex {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[LaTeX] Processing: {fn}");

            var pdfOut = Path.Combine(outDir, bn + ".pdf");
            var r = await ProcessRunner.RunAsync("pdflatex", $"\"{f}\" -o \"{pdfOut}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "latex", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "latex", ct)).ToArray()
        });
