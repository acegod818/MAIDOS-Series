using Forge.Core;
// MAIDOS-Forge SQL Language Plugin
// Code-QC v2.2B Compliant | M11 Specialist Plugin - Database Languages

using System.Text;
using Forge.Core.Config;
using Forge.Core.Platform;

namespace Forge.Plugin.Sql;

/// <summary>SQL 語言插件 - SQL database queries</summary>
public sealed class SqlPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".sql" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("sqlite3"))
            return (false, "sqlite3 not found. Install from https://www.sqlite.org/");
        var version = await ProcessRunner.GetVersionAsync("sqlite3", "--version");
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[SQL] Processing: {fn}");

            var r = await ProcessRunner.RunAsync("sqlite3", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "sql", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "sql", ct)).ToArray()
        });
