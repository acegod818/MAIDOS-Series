using Forge.Core;
// MAIDOS-Forge Clojure Language Plugin
...
namespace Forge.Plugin.Clojure;

/// <summary>Clojure language plugin</summary>
public sealed class ClojurePlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".clj", ".cljs", ".cljc" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("clojure"))
            return (false, "clojure not found. Install from https://clojure.org/guides/install_clojure");
        var version = await ProcessRunner.GetVersionAsync("clojure", "--version");
        return (true, $"clojure {version}");
    }
    ...
    public async Task<CompileResult> CompileAsync(ValidatedModuleConfig module, CompileConfig config, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            logs.Add($"[Clojure] Validating: {fn}");

            var r = await ProcessRunner.RunAsync("clojure", $"\"{f}\"",
    ...
    public async Task<InterfaceDescription?> ExtractInterfaceAsync(string artifactPath, CancellationToken ct = default)
        => new InterfaceDescription
        {
            ...
            Language = new InterfaceLanguage { Name = "clojure", Abi = "native" },
            Exports = (await NativeSymbolExtractor.ExtractFromBinaryAsync(artifactPath, "clojure", ct)).ToArray()
        });
