...
namespace Forge.Plugin.Zig;

/// <summary>Zig language plugin</summary>
public sealed class ZigPlugin : ILanguagePlugin
{
    private static readonly string[] SourceExtensions = { ".zig" };
    ...
    {
        if (!await ProcessRunner.CommandExistsAsync("zig"))
            return (false, "zig not found. Install from https://ziglang.org/download/");
        var version = await ProcessRunner.GetVersionAsync("zig", "version");
        return (true, $"zig {version}");
    }
