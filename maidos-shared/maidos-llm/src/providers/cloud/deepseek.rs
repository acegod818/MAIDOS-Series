...
//!
//! <impl>
//! WHAT: DeepSeek API implementation (OpenAI-compatible)
//! WHY: High performance, cost-effective Chinese AI models
...
impl LlmProvider for DeepSeekProvider {
    ...
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        if !response.status().is_success() {
            ...
            if status.as_u16() == 401 {
                return Err(LlmError::Auth("Invalid DeepSeek API key".to_string()));
            }
    ...
    async fn complete_stream(&self, _request: CompletionRequest) -> Result<CompletionStream> {
        Err(LlmError::Provider("Streaming not yet implemented for DeepSeek".to_string()))
    }
...
mod tests {
    ...
    fn test_build_request() {
        ...
        assert_eq!(api_req.model, "deepseek-chat");
        assert_eq!(api_req.messages.len(), 2);
    }

    #[test]
    fn test_custom_base_url() {
        let provider = DeepSeekProvider::new("key", Some("https://custom.api.com".to_string()));
        assert_eq!(provider.base_url, "https://custom.api.com");
    }

    #[test]
    fn test_default_base_url() {
        let provider = DeepSeekProvider::new("key", None);
    ...
    fn test_reasoner_model() {
        let provider = DeepSeekProvider::new("key", None);
        let info = provider.info();
        assert!(info.models.iter().any(|m| m.id == "deepseek-reasoner"));
    }
}
