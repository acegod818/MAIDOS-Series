...
var EXTENSION_TO_LANGUAGE, SUPPORTED_EXTENSIONS;
var init_languages = __esm({
  "src/languages.ts"() {
    ...
    SUPPORTED_EXTENSIONS = Object.keys(EXTENSION_TO_LANGUAGE);
  }
});

// src/z-axis.ts
var init_z_axis = __esm({
  "src/z-axis.ts"() {
  }
});
...
var GATE_CIRCUIT_LABELS, PROTECTION_LEVELS;
var init_hardwarization = __esm({
  "src/hardwarization.ts"() {
    GATE_CIRCUIT_LABELS = {
...
var AXIOMS, AXIOMS_BY_PRIORITY;
var init_b_axioms = __esm({
  "src/rules/b-axioms.ts"() {
    AXIOMS = [
      {
        id: "A1",
    ...
    AXIOMS_BY_PRIORITY = [...AXIOMS].sort((a, b) => a.priority - b.priority);
  }
});
...
var REDLINES;
var init_b_redlines_defs = __esm({
  "src/rules/b-redlines-defs.ts"() {
    REDLINES = [
...
var init_b_redlines_utils = __esm({
  "src/rules/b-redlines-utils.ts"() {
  }
});

// src/rules/b-redlines-r01-r05.ts
var CREDENTIAL_PATTERNS, CREDENTIAL_WHITELIST, R01_CHECKER, BYPASS_SECURITY_PATTERNS, R02_CHECKER, DELETE_AUDIT_PATTERNS, R03_CHECKER, UNAUTHORIZED_ACCESS_PATTERNS, R04_CHECKER, EMPTY_CATCH_PATTERNS, R05_CHECKER;
var init_b_redlines_r01_r05 = __esm({
  "src/rules/b-redlines-r01-r05.ts"() {
    init_b_redlines_defs();
    CREDENTIAL_PATTERNS = [
    ...
    CREDENTIAL_WHITELIST = [/process\.env\./, /os\.environ/, /env\s*\(/, /getenv\s*\(/, /\$\{[A-Z_]+\}/, /\$[A-Z_]+/];
    R01_CHECKER = {
      rule: getRedline("R01"),
...
var DIRECT_PROD_PATTERNS, PROD_FILE_WHITELIST, R06_CHECKER, SECURITY_DISABLE_PATTERNS, R07_CHECKER, VULNERABLE_PATTERNS, R08_CHECKER, UNLIMITED_RESOURCE_PATTERNS, R09_CHECKER, PLAINTEXT_PATTERNS, R10_CHECKER, SKIP_REVIEW_PATTERNS, R11_CHECKER, FAKE_TEST_PATTERNS, R12_CHECKER;
var init_b_redlines_r07_r12 = __esm({
  "src/rules/b-redlines-r07-r12.ts"() {
    init_b_redlines_defs();
    DIRECT_PROD_PATTERNS = [
      // Production DB 連線字串
    ...
    PROD_FILE_WHITELIST = /(?:migration|migrate|seed|schema|fixture|test|spec|mock)/i;
    R06_CHECKER = {
      rule: getRedline("R06"),
      checkSource(source, file) {
        const violations = [];
...
var FAKE_IMPL_PATTERNS, R13_CHECKER, SILENT_FAIL_PATTERNS, R14_CHECKER, TODO_PATTERNS, R15_CHECKER, EMPTY_METHOD_PATTERNS, R16_CHECKER, FRAUD_OBJECT_PATTERNS, R17_CHECKER, BYPASS_IMPL_PATTERNS, R18_CHECKER;
var init_b_redlines_r13_r18 = __esm({
  "src/rules/b-redlines-r13-r18.ts"() {
    init_b_redlines_defs();
    init_b_redlines_utils();
    FAKE_IMPL_PATTERNS = [
      /\btodo!\s*\(/g,
      /\bunimplemented!\s*\(/g,
...
var HARDCODED_RETURN_PATTERNS, R19_CHECKER, TEMPLATE_MARKER_PATTERNS, R20_CHECKER, R21_CHECKER, R22_CHECKER, ALWAYS_FAIL_PATTERNS, R23_CHECKER, SELF_CONFESSING_PATTERNS, R24_CHECKER;
var init_b_redlines_r19_r24 = __esm({
  "src/rules/b-redlines-r19-r24.ts"() {
    init_b_redlines_defs();
    HARDCODED_RETURN_PATTERNS = [
...
var TRIVIAL_RETURNS_RS, LOG_LINE_RS, LOG_LINE_CS, LOG_LINE_CPP, LOG_ONLY_WHITELIST, R25_CHECKER, R26_CHECKER, HIGH_RISK_CONTEXTS, ERROR_LAUNDER_PATTERNS, R27_CHECKER, CLONE_ARMY_PATTERNS, R28_CHECKER;
var init_b_redlines_r25_r28 = __esm({
  "src/rules/b-redlines-r25-r28.ts"() {
    init_b_redlines_defs();
    TRIVIAL_RETURNS_RS = /* @__PURE__ */ new Set([
      "Ok(())",
      "Ok(true)",
      "Ok(false)",
      "Ok(None)",
    ...
    LOG_LINE_RS = /^\s*(?:log|tracing|println|eprintln|debug|info|warn|error|trace)(?:::)?\w*!?\s*\(/;
    LOG_LINE_CS = /^\s*(?:Console\.Write|Debug\.Log|Logger\.\w+|_?logger\.\w+|Log\.\w+)\s*\(/;
    LOG_LINE_CPP = /^\s*(?:std::cout|std::cerr|printf|fprintf|spdlog|LOG_|SPDLOG_)\s*[(<]/;
    LOG_ONLY_WHITELIST = /* @__PURE__ */ new Set(["drop", "fmt", "display", "debug", "new", "default"]);
    R25_CHECKER = {
      rule: getRedline("R25"),
      checkSource(source, file) {
        const violations = [];
    ...
    HIGH_RISK_CONTEXTS = /(?:verify|auth|sign|encrypt|decrypt|download|install|delete|backup|restore|payment|transfer|checksum|hash|validate|wallet|key|secret|credential|private|password|token_refresh|certificate)/i;
    ERROR_LAUNDER_PATTERNS = [
      /\.unwrap_or_default\(\)/g,
...
var REDLINE_CHECKERS, ANTI_FRAUD_CHECKERS;
var init_b_redlines = __esm({
  "src/rules/b-redlines.ts"() {
    init_b_redlines_defs();
    init_b_redlines_utils();
    init_b_redlines_r01_r05();
    init_b_redlines_r07_r12();
    init_b_redlines_r13_r18();
    init_b_redlines_r19_r24();
    init_b_redlines_r25_r28();
    REDLINE_CHECKERS = [
      R01_CHECKER,
      R02_CHECKER,
      R03_CHECKER,
...
var THRESHOLDS, PROHIBITIONS, P03_CHECKER, MAGIC_NUMBER_PATTERN, ALLOWED_NUMBERS, P04_CHECKER, P05_CHECKER, P06_CHECKER, GLOBAL_STATE_PATTERNS, P07_CHECKER, MEANINGLESS, P09_CHECKER, P10_CHECKER, COMMENTED_CODE_PATTERNS, P12_CHECKER, P13_CHECKER, P14_CHECKER, P01_CHECKER, PREMATURE_OPT_PATTERNS, CUSTOM_DS_PATTERN, MEMORY_POOL_PATTERN, INLINE_ASM_PATTERN, P02_CHECKER, P08_CHECKER, SQL_PATTERN, UI_PATTERN, HTTP_CLIENT_PATTERN, LOW_LEVEL_IO_PATTERN, P11_CHECKER, PROHIBITION_CHECKERS;
var init_b_prohibitions = __esm({
  "src/rules/b-prohibitions.ts"() {
    THRESHOLDS = {
      /** P03: 重複代碼組數 */
      DUPLICATE_GROUPS: 3,
    ...
    MAGIC_NUMBER_PATTERN = /(?<![\w.])\b(\d{2,})\b(?!\s*[,\]:}])/g;
    ALLOWED_NUMBERS = /* @__PURE__ */ new Set(["0", "1", "2", "10", "100", "1000", "60", "24", "365", "1024", "4096"]);
    P04_CHECKER = {
    ...
    MEANINGLESS = ["temp", "tmp", "data", "info", "val", "value", "result", "res", "obj", "item", "foo", "bar", "baz", "x", "y", "z", "a", "b", "c"];
    P09_CHECKER = {
    ...
    PREMATURE_OPT_PATTERNS = [
      // 位運算代替基本算數
      /\b\w+\s*(?:<<|>>)\s*1\b/g,
      /\b\w+\s*&\s*1\b/g
    ];
    CUSTOM_DS_PATTERN = /\bclass\s+(?:LinkedList|BTree|BPlusTree|HashMap|HashSet|RedBlackTree|SkipList|Trie|AVLTree)\b/gi;
    MEMORY_POOL_PATTERN = /\bclass\s+(?:ObjectPool|MemoryPool|BufferPool|ThreadPool|ConnectionPool)\b/gi;
    INLINE_ASM_PATTERN = /\b(?:__asm__|asm\s*\(|__attribute__\s*\(\s*\(\s*always_inline|#\[inline\(always\)\])/g;
    P02_CHECKER = {
    ...
    SQL_PATTERN = /\b(?:SELECT\s+\S|INSERT\s+INTO\b|UPDATE\s+\w+\s+SET\b|DELETE\s+FROM\b)/i;
    UI_PATTERN = /(?:document\.\w|innerHTML|querySelector|getElementById|React\.\w|\.render\s*\(|createElement\b)/i;
    HTTP_CLIENT_PATTERN = /\b(?:fetch\s*\(|axios\.|http\.(?:get|post|put|delete)|HttpClient|XMLHttpRequest|reqwest::)/i;
    LOW_LEVEL_IO_PATTERN = /\b(?:fs\.(?:read|write|open|close)|File\.(?:open|read|write)|fopen\s*\(|fread\s*\(|std::fs::)/i;
    P11_CHECKER = {
...
var GATES, X_WEIGHTS, Y_WEIGHTS, HANDOVER_TEMPLATE, HANDOVER_TAGS;
var init_c_gates = __esm({
  "src/rules/c-gates.ts"() {
    GATES = [
      {
        id: "Gate-In",
...
__export(rules_exports, {
  ANTI_FRAUD_CHECKERS: () => ANTI_FRAUD_CHECKERS,
  AXIOMS: () => AXIOMS,
  AXIOMS_BY_PRIORITY: () => AXIOMS_BY_PRIORITY,
  GATES: () => GATES,
  HANDOVER_TAGS: () => HANDOVER_TAGS,
  HANDOVER_TEMPLATE: () => HANDOVER_TEMPLATE,
  PROHIBITIONS: () => PROHIBITIONS,
  PROHIBITION_CHECKERS: () => PROHIBITION_CHECKERS,
  REDLINES: () => REDLINES,
  REDLINE_CHECKERS: () => REDLINE_CHECKERS,
  calculateComplianceScore: () => calculateComplianceScore,
  calculateDualAxisScore: () => calculateDualAxisScore,
  calculateOutcomeScore: () => calculateOutcomeScore,
  checkAntifraud: () => checkAntifraud,
  checkFraud: () => checkFraud,
  checkProhibitions: () => checkProhibitions,
  checkRedlines: () => checkRedlines,
  checkRules: () => checkRules,
  createGateStatus: () => createGateStatus,
  evaluateGate: () => evaluateGate,
  extractHandoverTags: () => extractHandoverTags,
  formatAxiomsPrompt: () => formatAxiomsPrompt,
  generateAllGatesChecklist: () => generateAllGatesChecklist,
  generateGateChecklist: () => generateGateChecklist,
  getAllRules: () => getAllRules,
  getAxiom: () => getAxiom,
  getProhibition: () => getProhibition,
  getRedline: () => getRedline
});
...
var C_LIKE_LANGUAGES;
var init_analyzer = __esm({
  "src/analyzer.ts"() {
    init_rules();
    init_languages();
    C_LIKE_LANGUAGES = /* @__PURE__ */ new Set([
      // Core
      "typescript",
      "javascript",
      "rust",
...
__export(evidence_exports, {
  ...
  generateProofPackManifest: () => generateProofPackManifest,
  judgeDod: () => judgeDod
});
...
var init_protection = __esm({
  "src/engine/protection.ts"() {
    init_types();
    LV_CHECKERS = [checkLV1, checkLV2, checkLV3, checkLV4, checkLV5, checkLV6, checkLV7, checkLV8, checkLV9];
  }
});
...
__export(pipeline_exports, {
  formatPipelineReport: () => formatPipelineReport,
...
function runPipeline(input) {
  ...
  const nonce = input.nonce && input.nonce.trim().length > 0 ? input.nonce.trim() : crypto.randomUUID();
...
__export(app_exports, {
  ...
  startServerWithHandle: () => startServerWithHandle
});
...
async function handlePipeline(req, res) {
  const body = JSON.parse(await readBody(req));
  const { files, projectPath = ".", grade = "E" } = body;
  if (!files || !Array.isArray(files)) {
    return json(res, 400, { error: "files array required" });
  }
  const ctx = {
...
async function handleFraud(req, res) {
  const body = JSON.parse(await readBody(req));
  const { files } = body;
  if (!files || !Array.isArray(files)) {
    return json(res, 400, { error: "files array required" });
...
function handleRules(_req, res) {
  Promise.resolve().then(() => (init_rules(), rules_exports)).then((mod) => {
    json(res, 200, {
      ok: true,
...
function getDashboardPath() {
  ...
  try {
    ...
    const candidate = path.resolve(distDir, "..", "web-ui", "dashboard.html");
    if (fs.existsSync(candidate)) return candidate;
  } catch {
  }
  return path.resolve(process.cwd(), "web-ui", "dashboard.html");
}
...
function startServer(config = {}) {
  ...
  return new Promise((resolve3) => {
    const server = http.createServer((req, res) => router(req, res, cfg));
    server.listen(cfg.port, cfg.host, () => {
      console.log(`
\u{1F527} MAIDOS CodeQC API v${VERSION} (Code-QC v${CODEQC_VERSION})`);
...
async function startServerWithHandle(config = {}) {
  const cfg = { ...DEFAULT_SERVER_CONFIG, ...config };
  return new Promise((resolve3) => {
    const server = http.createServer((req, res) => router(req, res, cfg));
    server.listen(cfg.port, cfg.host, () => resolve3(server));
  });
}
...
async function runExternalCmd(cmd, cwd) {
  const { execSync } = await import('child_process');
  try {
    const log = execSync(cmd, { cwd, encoding: "utf-8", timeout: 12e4, stdio: ["pipe", "pipe", "pipe"] });
...
function parseCoverage(log) {
  ...
  if (headerIdx !== -1 && totalIdx !== -1) {
    ...
    const coverIdxs = headerParts.map((p, i) => ({ p, i })).filter((x) => x.p === "Cover").map((x) => x.i);
...
function parseIavLog(content) {
  ...
  return {
    ...
    passedCount,
    failedCount,
    details: passed ? `IAV PASS (${passedCount} points)` : `IAV FAIL (points=${verdicts.length}, pass=${passedCount}, fail=${failedCount}, qOk=${qOk})`
  };
}
function parseBldsLog(content, threshold) {
  const totals = [];
  ...
  return {
    ...
    minScore,
    threshold,
    details: passed ? `BLDS PASS (min=${minScore}/5 >= ${threshold})` : `BLDS FAIL (min=${minScore}/5, need>=${threshold}, points=${totals.length})`
  };
}
function parseDatasourceLog(content) {
  ...
  return {
    passed,
    untraced,
    details: passed ? `DATASOURCE PASS (lines=${lines.length})` : `DATASOURCE FAIL (lines=${lines.length}, untraced=${untraced})`
  };
}
async function detectPackageScripts(targetPath) {
  const { readFileSync: readFileSync3, existsSync: existsSync3 } = await import('fs');
  const { resolve: resolve3 } = await import('path');
  const scripts = {};
  const pkgPath = resolve3(targetPath, "package.json");
...
`);
  ...
  await startServer2({ port, apiKey });
}
async function pipelineCommand(args, loadFiles2) {
  const { runPipeline: runPipeline2, formatPipelineReport: formatPipelineReport2 } = await Promise.resolve().then(() => (init_pipeline(), pipeline_exports));
  const { generateProofPackManifest: generateProofPackManifest2, collectEvidence: collectEvidence2 } = await Promise.resolve().then(() => (init_evidence(), evidence_exports));
  const { writeFileSync: writeFileSync2, mkdirSync, existsSync: existsSync3 } = await import('fs');
  const { resolve: resolve3 } = await import('path');
  if (args.includes("-h") || args.includes("--help")) {
    console.log(`
...
`);
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--grade" || arg === "-g") {
      ...
      if (g === "E" || g === "F") grade = g;
    } else if (arg === "--build-cmd") {
      buildCmd = args[++i];
    } else if (arg === "--test-cmd") {
      testCmd = args[++i];
    } else if (arg === "--lint-cmd") {
      lintCmd = args[++i];
    } else if (arg === "--coverage-cmd") {
      coverageCmd = args[++i];
    } else if (arg === "--audit-cmd") {
      auditCmd = args[++i];
    } else if (arg === "--package-cmd") {
      packageCmd = args[++i];
    } else if (arg === "--run-cmd") {
      runCmd = args[++i];
    } else if (arg === "--spec") {
      ...
      autoDetect = false;
    } else if (arg === "--full") ; else if (!arg.startsWith("-")) {
      target = arg;
    }
  }
  const absTarget = resolve3(target);
  console.log(`
\u{1F527} CodeQC v3.5 Pipeline \u2014 Grade ${grade}`);
...
`);
  ...
  if (buildCmd) {
    console.log(`  \u{1F528} Build: ${buildCmd}`);
...
`);
  ...
  if (lintCmd) {
    console.log(`  \u{1F9F9} Lint: ${lintCmd}`);
...
`);
  ...
  if (testCmd) {
    console.log(`  \u{1F9EA} Test: ${testCmd}`);
...
`);
  ...
  if (coverageCmd) {
    console.log(`  \u{1F4CA} Coverage: ${coverageCmd}`);
...
`);
  ...
  if (auditCmd) {
    console.log(`  \u{1F50E} Audit: ${auditCmd}`);
...
`);
  ...
  if (packageCmd) {
    console.log(`  \u{1F4E6} Package: ${packageCmd}`);
...
`);
  ...
  const rawFiles = loadFiles2(target);
  const files = rawFiles.map((f) => ({ path: f.path, content: f.content }));
  let specFunctions;
  let specChecklist;
  if (specPath && existsSync3(resolve3(absTarget, specPath))) {
    const specContent = (await import('fs')).readFileSync(resolve3(absTarget, specPath), "utf-8");
  ...
  const evidenceDir = resolve3(absTarget, "evidence");
  try {
    mkdirSync(evidenceDir, { recursive: true });
  } catch {
  }
  let proof;
  const iavPath = resolve3(evidenceDir, "iav.log");
  const bldsPath = resolve3(evidenceDir, "blds.log");
  const dsPath = resolve3(evidenceDir, "datasource.log");
  const iavContent = existsSync3(iavPath) ? (await import('fs')).readFileSync(iavPath, "utf-8") : void 0;
  const bldsContent = existsSync3(bldsPath) ? (await import('fs')).readFileSync(bldsPath, "utf-8") : void 0;
  const dsContent = existsSync3(dsPath) ? (await import('fs')).readFileSync(dsPath, "utf-8") : void 0;
  ...
  const pipelineResult = runPipeline2({
    targetPath: target,
    files,
    grade,
    evidenceDir,
    externalResults: Object.keys(externalResults).length > 0 ? externalResults : void 0,
    specPath,
    specFunctions,
    specChecklist,
    proof,
  ...
  console.log(formatPipelineReport2(pipelineResult));
  for (const [name, data] of Object.entries(externalResults)) {
    const val = data;
    if (val.log) {
      writeFileSync2(resolve3(evidenceDir, `${name}.log`), val.log, "utf-8");
    }
  }
  for (const step of pipelineResult.steps) {
    if (!step.evidencePath) continue;
    if (typeof step.log !== "string") continue;
    const p = resolve3(absTarget, step.evidencePath);
    writeFileSync2(p, step.log, "utf-8");
  }
  writeFileSync2(resolve3(evidenceDir, "g1.log"), `G1 ${pipelineResult.gates.G1.passed ? "PASS" : "FAIL"}
${pipelineResult.gates.G1.details}
`, "utf-8");
  writeFileSync2(resolve3(evidenceDir, "g2.log"), `G2 ${pipelineResult.gates.G2.passed ? "PASS" : "FAIL"}
${pipelineResult.gates.G2.details}
`, "utf-8");
  writeFileSync2(resolve3(evidenceDir, "g3.log"), `G3 ${pipelineResult.gates.G3.passed ? "PASS" : "FAIL"}
${pipelineResult.gates.G3.details}
`, "utf-8");
  writeFileSync2(resolve3(evidenceDir, "g4.log"), `G4 ${pipelineResult.gates.G4.passed ? "PASS" : "FAIL"}
...
`, "utf-8");
  ...
  writeFileSync2(resolve3(evidenceDir, "impl.log"), implLines.join("\n") + "\n", "utf-8");
  writeFileSync2(resolve3(evidenceDir, "nonce.log"), pipelineResult.nonce + "\n", "utf-8");
  writeFileSync2(resolve3(evidenceDir, "hash.log"), pipelineResult.evidenceHash + "\n", "utf-8");
  const evidence = collectEvidence2(pipelineResult.steps, pipelineResult.gates, { evidenceDir, files, externalResults, specChecklist, proof });
  const manifest = generateProofPackManifest2(evidence);
  const reportPath = resolve3(evidenceDir, "PROOF_PACK.md");
  writeFileSync2(reportPath, manifest, "utf-8");
  console.log(`  \u{1F4C4} Proof Pack: ${reportPath}
`);
  ...
  writeFileSync2(resolve3(evidenceDir, "PROOF-REPORT.md"), proofReportLines.join("\n"), "utf-8");
...
init_languages();
var DEFAULT_CONFIG2 = {
  categories: /* @__PURE__ */ new Set(["security", "structure", "quality"]),
  rules: {
    security: {
      credentials: true,
      injection: true,
...
function parseArgs(args, base) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "-h" || arg === "--help") {
      ...
      result.categories = /* @__PURE__ */ new Set();
      ...
      if (result.categories.size === 0) {
        result.categories = /* @__PURE__ */ new Set(["security", "structure", "quality"]);
      }
      ...
      result.categories = /* @__PURE__ */ new Set(["security"]);
    } else if (arg === "--only-structure") {
      result.categories = /* @__PURE__ */ new Set(["structure"]);
    } else if (arg === "--only-quality") {
      result.categories = /* @__PURE__ */ new Set(["quality"]);
...
function normalizeCategory(raw) {
  ...
  if (v === "all" || v === "a") return null;
  return null;
}
function normalizeLevel(raw) {
  const v = String(raw ?? "").trim().toUpperCase();
  return v === "B" || v === "C" || v === "D" ? v : null;
}
function normalizeReporter(raw) {
  const v = String(raw ?? "").trim().toLowerCase();
  return v === "console" || v === "json" || v === "html" ? v : null;
}
async function loadCliConfig(configPath) {
  ...
  if (cfg.categories) {
    const rawCats = Array.isArray(cfg.categories) ? cfg.categories.map(String) : String(cfg.categories).split(",");
...
function discoverFiles(targetPath) {
  ...
  function walk(dir) {
    ...
    try {
      ...
      const codeVal = err && typeof err === "object" && "code" in err ? String(err.code) : "";
...
function filterByCategory(result, categories) {
...
async function main() {
  ...
  const base = cfgPath ? await loadCliConfig(cfgPath) : void 0;
